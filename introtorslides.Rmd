...
I'm going to talk a little bit about R's origins, how it compares to other languages, notably python, which is the other language that tends to be used as a statistical programming language. I'll talk a bit about the extensive amount of code libraries that are out there, which in R-speak are called packages. I'll talk a little bit about R's place in  analysis pipelines, talk though a few pointers on  learning resources, and finish up with a couple of examples of things I’ve done in R @ NICE. 
...
So how might you typically use R in an analysis pipeline?

So in an analysis pipeline you’d use SQL or similar query language for the heavy lifting work getting data out of a database, then ideally leaving R or Python to run your statistical analysis, exporting results and data visualisations. 

Once you have your data, it’ll be stored in a dataframe object, equivalent to a python pandas dataframe, or database table. This gives your data a bit of structure, names each column etc so you can run operations on particular columns and rows using simpler syntax. You might end up doing some data carpentry or manipulation in R, which is fine, as long as the data source can be loaded into memory on the workstation you're running R on.
...
After you’ve got results for that, you can use R to generate graphics, which are really good. For instance, the BBC use R for their website graphics. You can also use R to create pdfs, word documents or HTML pages. You can even create an online slide deck like this. You can also use the R Shiny framework to create a web app or dashboard as a front-end for the analysis. Or you can just run off a flat file of results or feed results to a downstream service, dependent on what you want to get out.
...
Hopefully that’s piqued your interest a little bit to learn more, so I've included a few resources for you on this slide
